...
public class PostgresRentRepository implements RentBookRepository {
    ...
    public void saveOrUpdate(RentBook rentBook) {
        ...
        if (optionalRentBook.isPresent()) {
            String sql = "UPDATE TESTDB.RENT_BOOK set age = ?" +
                    "SET BOOK_ID = ?," +
                    "CLIENT_ID = ?," +
                    "RENT_PRICE = ?," +
                    "INIT_DATE = ?," +
            ...
            jdbcTemplate.update(
                    "INSERT INTO TESTDB.RENT_BOOK (BOOK_ID,CLIENT_ID,RENT_PRICE,INIT_DATE,END_DATE,RETURNED) VALUES ( ?, ?, ?, ?, ?, ?)", rentBook.getBookId(), rentBook.getClientId(), rentBook.getRentPrice(), rentBook.getInitDate(), rentBook.getEndDate(), rentBook.getReturned());
        }
    ...
    public Optional<RentBook> findById(Integer id) {
        String sql = "SELECT * FROM TESTDB.RENT_BOOK WHERE id = ?";
    ...
    public Boolean isRented(Integer bookId, Integer clienteId) {
        String sql = "SELECT * FROM TESTDB.RENT_BOOK WHERE BOOK_ID = ?";
